...
export default function CellsView({ initialCells }: { initialCells: Cell[] }) {
  ...
  const handleUpdateCell = async (cell: Cell) => {
    const name = prompt('Hücre (Departman) Adı:', cell.name);
    if (!name) return;

    try {
      const result = await updateCellAction(cell.id, name);
      if (result.success && result.cell) {
        toast.success('Hücre güncellendi.');
        setCells(prev => prev.map(c => c.id === cell.id ? { ...c, name: result.cell.name } : c));
      } else {
        toast.error(result.error || 'Hücre güncellenemedi.');
      }
    } catch (err) {
  ...
  const handleDeleteCell = async () => {
    ...
    setIsDeleting(true);
    try {
      const result = await deleteCellAction(deletingCell.id);
      if (result.success) {
        toast.success('Hücre silindi.');
        setCells(prev => prev.filter(c => c.id !== deletingCell.id));
        setDeletingCell(null);
      } else {
        toast.error(result.error || 'Hücre silinemedi.');
      }
    } catch (err) {
      toast.error('Hata oluştu.');
    } finally {
  ...
  return (
    <>
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  {canManage && (
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => handleUpdateCell(cell)}
                        className="p-3 bg-gray-50 hover:bg-amber-50 hover:text-amber-600 text-gray-400 rounded-2xl transition-all"
                      >
                        <Edit2 size={18} />
                      </button>
                      <button
                        onClick={() => setDeletingCell(cell)}
                        className="p-3 bg-gray-50 hover:bg-red-50 hover:text-red-600 text-gray-400 rounded-2xl transition-all"
                      >
                        <Trash2 size={18} />
                      </button>
                      <button
      ...
      {deletingCell && (
        <div className="fixed inset-0 z-[110] bg-gray-950/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-white rounded-[2.5rem] p-10 max-w-md w-full shadow-2xl border border-gray-100 flex flex-col items-center text-center">
            <div className="w-20 h-20 bg-red-50 rounded-full flex items-center justify-center text-red-600 mb-6">
              <Trash2 size={40} />
            </div>
            <h3 className="text-2xl font-black text-gray-900 mb-2">Hücreyi Sil?</h3>
            <p className="text-gray-500 font-medium mb-8 leading-relaxed">
              <span className="font-bold text-gray-900">"{deletingCell?.name}"</span> hücresini silmek istediğinize emin misiniz? <br />
              <span className="text-xs text-red-500 bg-red-50 px-2 py-1 rounded-lg inline-block mt-2 font-black uppercase tracking-tighter">
                DİKKAT: Bağlı istasyonlar ve projeler boşa düşecektir!
              </span>
            </p>
            <div className="flex gap-4 w-full">
              <button
                onClick={() => setDeletingCell(null)}
                className="flex-1 py-4 bg-gray-100 hover:bg-gray-200 text-gray-900 rounded-2xl font-black transition-all"
              >
                İPTAL
              </button>
              <button
                ...
                disabled={isDeleting}
                className="flex-1 py-4 bg-red-600 hover:bg-red-700 text-white rounded-2xl font-black shadow-lg shadow-red-500/20 transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {isDeleting ? <Loader2 size={18} className="animate-spin" /> : 'SİL'}
              </button>
            </div>
          </div>
        </div>
